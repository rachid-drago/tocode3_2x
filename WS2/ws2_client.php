	...
	if(isset($_POST['sub'])){
		...
		if(!$error){
			...
			if ($err) {
			 ...
			 try {
                 echo $id;
                $result = $client->call('FullCountryInfo', array('sCountryISOCode'=>$id));
			...
			if ($err) {
			 ...
			 try {
				/** Call insert book method */
				 $response =  $client->call('FullCountryInfoSoap', array($id));
  ...
  <table class="table">
    ...
    <?php if($result){ ?>
		      ...
		      <tr>
                ...
                <td><?php echo '<img style="height:100px;width:150px"src="'. $result['FullCountryInfoResult']['sCountryFlag'].'"/>'; ?></td>
